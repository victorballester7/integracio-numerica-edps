...
#define um(j, i) um[(gr->nx + 1) * (j) + (i)]
#define aux(j, i) aux[(gr->nx + 1) * (j) + (i)]
  ...
  for (int m = 1; m < maxit && errorLoo > tol; m++) {
    memcpy(aux, um, (NX + 1) * (NY + 1) * sizeof(double));
    for (int i = 1; i < NX; i++) {
      for (int j = 1; j < NY; j++) {
    ...
    for (int i = 1; i < NX; i++) {
      for (int j = 1; j < NY; j++)
        errorLoo = MAX(errorLoo, fabs(um(j, i) - aux(j, i)));
    }
  }

% ----------------------------------------------------------------------- %
% Implements the Lax-Friedrichs scheme for the 1D wave equation u_t + a u_x = 0 with inital condition u(0,x) = u0(x) and boundary conditions u(t,x0) = 0. Also it prints the error between the numerical solution and the exact solution.
%
% Parameters:
%   a: wave speed
%   h: spatial step size
...
function [u, errLinf, errL2] = lf(a, h, lamb, x0, x1, t1)
  ...
  n = t1/k;
  for i = 1:n
    u(2:end-1) = 0.5 * (u(3:end) + u(1:end-2) - a*lamb*(u(3:end) - u(1:end-2)));
    u(1) = 0;
    u(end) = u(end-1);
  end
